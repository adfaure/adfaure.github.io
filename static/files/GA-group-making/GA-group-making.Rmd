...
fitness=function(x)
{
  ...
  m <- matrix(x, ncol=nb_students, byrow=TRUE)
...
GA=ga(type='binary',fitness=fitness,nBits=nb_students*max_groups,maxiter=30,popSize=100, pcrossover=0.1, pmutation = 0.8, seed=seed, keepBest=TRUE)
summary(GA)
plot(GA)
```
...
func <- function(sol){
  ...
  counts <- c()
  for(i in 1:6){
    if(length(test[test == i]) > 0)
      counts[as.character(i)] <- length(test[test == i])
  }
  print(counts)
